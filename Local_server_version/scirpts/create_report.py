...
def get_QC_png(sequencing_type,input_dir,out_dir):
    ...
    if sequencing_type=='ATAC':
        ...
        for path in glob.glob(input_dir+'/step1_fastqc_and_trim_galore/trim_galore/*/*.zip'):
            temp=path.split('/')
            sample_name=temp[-2]
            R1_or_R2='R1' if temp[-1].split('_')[3]=='1' else 'R2'
            zip_file=zipfile.ZipFile(path)
            zip_list=zip_file.namelist()
            for f in zip_list:
                if 'per_base_quality.png' in f:
...
def create_report(name,sequencing_type,input_dir,out_dir,logo_image_path='/picb/rsgeno/chenhj/test_docker_images/logo_and_cover_image/logo.png',cover_image_path='/picb/rsgeno/chenhj/test_docker_images/logo_and_cover_image/cover.png'):
    geometry_options={
        "head":"40pt",
        "margin":"0.5in",
        "bottom":"0.6in",
        ...
        with doc.create(Subsection('The figures below show the reads distribution across gene body.')):
            ...
            with doc.create(LongTabu("X[2l] X[2l] X[2l] X[2l]",row_height=1.5)) as data_table:
                ...
                while index+step<=len(paths):
                    ...
                    index+=step
                if index<len(paths):
                    data_table.add_row([StandAloneGraphic(path,image_options="width=100px") for path in paths[index:len(paths)]]+['']*(len(paths)-index))
    ...
    with doc.create(Section('Differential analysis')): 
        ...
        for path in glob.glob(input_dir+'/step7_differential_analysis/*MA_plot.png'):
            analysis_ID='_'.join(path.split('/')[-1].split('_')[:3])
            png_dic[analysis_ID]={}
        ...
        with doc.create(LongTabu("X[2l] X[2l] X[2l]",row_height=1.5)) as data_table:
            ...
            for analysis_ID in png_dic:
                ...
                data_table.add_row([StandAloneGraphic(png_dic[analysis_ID]['MVC_plot'],image_options="width=180px"),StandAloneGraphic(png_dic[analysis_ID]['MA_plot'],image_options="width=180px"),StandAloneGraphic(png_dic[analysis_ID]['motif_enrichment_plot'],image_options="width=180px")])
...
def main():
    ...
    try:
        opts,args=getopt(argv[1:],'h',['input_dir=','name=','sequencing_type=','outdir=','cover_image=','logo_image=','help'])
        for i,j in opts:   
            if i=="-h" or i=="--help":
    ...
    create_report(name,sequencing_type,input_dir,out_dir)
